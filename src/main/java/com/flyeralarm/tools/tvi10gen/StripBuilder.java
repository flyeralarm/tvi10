...
public class StripBuilder {
     ...
     * Default constructor.
     */
    public StripBuilder() throws IOException, DocumentException {
        byte[] bytes;

        bytes = IOUtils.toByteArray(StripBuilder.class.getResourceAsStream(RES_FONT_REGULAR));
        fontRegular = BaseFont.createFont("regular.ttf", BaseFont.WINANSI, BaseFont.EMBEDDED, true, bytes, null);

        bytes = IOUtils.toByteArray(StripBuilder.class.getResourceAsStream(RES_FONT_BOLD));
